...
class PreProcessor(object):
    ...
    def trainy(self, testy=0.2, imbl=True):
        ...
        for i in np.arange(1,4):
            ...
            Xwords_train = tsvd.fit_transform(Xword_train)
            Xwords_test = tsvd.transform(Xword_test)
            Xwords_train_df = pd.DataFrame(Xwords_train,
                                            columns=[str(i)+'_'+str(b) for b in np.arange(1,Xwords_train.shape[1]+1)])
            Xwords_test_df = pd.DataFrame(Xwords_test,
                                           columns=[str(i)+'_'+str(b) for b in np.arange(1,Xwords_test.shape[1]+1)])
            df_train = pd.concat([df_train,Xwords_train_df], axis=1)
            df_test = pd.concat([df_test,Xwords_test_df], axis=1)
    ...
    def try_model(self, model, label):
        ...
        print(fitted.score(self.X_test, self.y_test))
...
class Classifier(object):
    ...
    def preprocess(self, input_string, web=True):
        ...
        weblink, words_only = self.extractions_mod(line=input_string, string='https?://.*') 
        if web:
            ...
            list(map(lambda x: self.sumdict.pop(x, None), ['n_video','n_links','n_image','mention_count','hashtag_count',
        ...
        if self.sumdict['n_word'] > 0:
            ...
            self.avgdict['word_cap_ratio'] = 0.0
            self.avgdict['char_cap_ratio'] = 0.0
            self.avgdict['med_char'] = 0.0
            self.avgdict['med_word'] = 0.0
            self.avgdict['word_char_ratio'] = 0.0
        ...
        for col in list(convtag_dict.keys()):
    ...
    def perform_magic(self, M):
        ...
        for i,(tfidf, tsvd) in enumerate(zip(M.tfidf_list, M.tsvd_list)):
            tsvd_df = tsvd.transform(tfidf.transform(chunkie))
            da_df = pd.DataFrame(tsvd_df, columns=[str(i+1)+'_'+str(b) for b in np.arange(1,tsvd_df.shape[1]+1)])
        ...
        magic = M.fitted.predict(self.test)
        return magic

    def perform_more_magic(self, M):
        return M.fitted.predict_proba(self.test)[0][M.fitted.predict(self.test)]
